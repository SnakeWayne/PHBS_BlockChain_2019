...
public class Transaction {
    ...
    public byte[] getRawTx() {
        ...
        //this part is import for block generation miner.Because if a miner mine 2 block,the he will have 2 coinbase block,
        // but without the random noise,the tx hash remains the same so the older one may be covered by the new one.
        Random random = new Random();
        byte[] randomnoise = random.doubles().toString().getBytes();
        for (int i = 0; i < randomnoise.length; i++)
            rawTx.add(randomnoise[i]);
